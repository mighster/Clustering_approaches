...
colnames(Merged_df)
rm(new_df)
new_df <- Merged_df[,c(3:7,9:13,60:98)]
row.names(new_df)
str(new_df)
colnames(new_df)
new_df[1:292,1:2]
...
tsne_data_Day9 <- tsne_obj$Y %>%
  data.frame() %>%
  setNames(c("X", "Y")) %>%
  mutate(cluster = factor(pam_fit$clustering),
         name = Merged_df$Name)

tiff("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/Gower/tSNE_Day9.tiff", units = "in" ,height = 8, width = 10, res=150)
ggplot(aes(x = X, y = Y), data = tsne_data_Day9) +
  geom_point(aes(color = cluster, size = 0.25))+
  ggtitle("tSNE Clustering at 9 Days")
dev.off()


write.csv(tsne_data_Day9,"C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/Gower/tsne_data_Day9.csv", row.names = T)
